...
class Html
{
    ...
    public static function getBody(string $html)
    : string
    {
        return self::getOuterHTML($html, 'body');
    }

    public static function hasHead(string $html)
    : bool
    {
        return strlen(self::getHead($html)) > 0;
    }

    public static function getHead(string $html)
    : string
    {
        return self::getOuterHTML($html, 'head');
    }

    public static function getOuterHTML(string $html, string $tagName)
    : string
    {
        $patterns = '/\<'.$tagName.'(.[^\>]*)\>([^\|]*)\<\/'.$tagName.'\>/';
        $matches = PregUtil::getMatches($patterns, $html);
        if (!empty($matches) && isset($matches[0]) && isset($matches[0][2])) {
            return $matches[0][0];
        }
        return '';
    }
